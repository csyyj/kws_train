...
def gen_kw_pickle_list(pickle_pathes):
        keys = []
...
def gen_speaker_list(speech_dir):
    wav_l = []
...
def get_long_wav(speaker_list, wav_len):
    while True:
        spk_id = random.randint(0, len(speaker_list) - 1)
        spk_path = speaker_list[spk_id]
        spk_wav = np.load(spk_path, mmap_mode='c')
        if spk_wav.size - 1 - wav_len > 0:
            break
    rdm_start = random.randint(0, spk_wav.size - 1 - wav_len)
