...
if __name__ == '__main__':
    ...
    resume_model(net_work, './model/student_model/model-842500-1.4094600439071656.pickle')
    net_work.eval()
    import soundfile as sf
    data, _ = sf.read('./process/wuling/aec.wav')
    with torch.no_grad():
        data_in = torch.from_numpy(data.astype(np.float32)).reshape(1, -1)
        est_logist = net_work(data_in)
        est_logist = est_logist.squeeze()[:, 1:].sum(-1)
