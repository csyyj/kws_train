...
def script_lstm(input_size, hidden_size, num_layers, bias=True,
                batch_first=False, dropout=False, bidirectional=False, decay=False, is_old=False):
    ...
    if not decay:
        ...
        if is_old:
            return stack_type(num_layers, layer_type,
                              first_layer_args=[DecayLSTMCellOld, input_size, hidden_size],
                              other_layer_args=[DecayLSTMCellOld, hidden_size * dirs, hidden_size])
        else:
            return stack_type(num_layers, layer_type,
                              first_layer_args=[DecayLSTMCell, input_size, hidden_size],
                              other_layer_args=[DecayLSTMCell, hidden_size * dirs, hidden_size])
...
class DecayLSTMCellOld(jit.ScriptModule):

    def __init__(self, input_size, hidden_size):
        super(DecayLSTMCellOld, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.weight_ih = Parameter(torch.randn(4 * hidden_size, input_size))
        self.weight_hh = Parameter(torch.randn(4 * hidden_size, hidden_size))
        self.bias_ih = Parameter(torch.randn(4 * hidden_size))
        self.bias_hh = Parameter(torch.randn(4 * hidden_size))
        self.decay_scale = Parameter(torch.randn(hidden_size))
    ...
    def forward(self, input, state):
        ...
        decay_rate = torch.sigmoid(self.decay_scale)
        cy = decay_rate * (forgetgate * cx) + (1 - decay_rate) * (ingate * cellgate)
        hy = outgate * torch.tanh(cy)

        return hy, (hy, cy)


class DecayLSTMCell(jit.ScriptModule):

    def __init__(self, input_size, hidden_size):
        super(DecayLSTMCell, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.weight_ih = Parameter(torch.randn(4 * hidden_size, input_size))
        self.weight_hh = Parameter(torch.randn(4 * hidden_size, hidden_size))
...
class LSTMLayer(jit.ScriptModule):
    def __init__(self, cell, *cell_args):
        ...
        self.reset_parameters(self.cell)
